...
export default function RequisitosRecuperacion() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <h2 className="text-2xl font-semibold">Consulta de Requisitos de Recuperación</h2>
      <p className="text-sm text-gray-700">
        Selecciona tu licenciatura para consultar los requisitos de recuperación.
      </p>
      ...
      {documento && (
        <div className="bg-white rounded-lg shadow p-6">
          <h3 className="font-semibold text-lg mb-4">Requisitos de Recuperación</h3>
          {documento.recuperacion ? (
            <div className="text-sm space-y-3">
              <div>
                <p className="font-semibold text-gray-700">Duración:</p>
                <p className="text-gray-600">{documento.recuperacion.duracion}</p>
              </div>
              <div>
                <p className="font-semibold text-gray-700">Material:</p>
                <p className="text-gray-600">{documento.recuperacion.material}</p>
              </div>
              <div>
                <p className="font-semibold text-gray-700">Notas:</p>
                <p className="text-gray-600">{documento.recuperacion.notas}</p>
              </div>
              <div>
                <p className="font-semibold text-gray-700">Requisitos:</p>
                <p className="text-gray-600">{documento.recuperacion.requisitos}</p>
              </div>
              {documento.recuperacion.espacio && (
                <div>
                  <p className="font-semibold text-gray-700">Espacio:</p>
                  <p className="text-gray-600">{documento.recuperacion.espacio.lugar}</p>
                  {documento.recuperacion.espacio.liga && (
                    <p className="text-sky-600 hover:underline">
                      <a href={documento.recuperacion.espacio.liga} target="_blank" rel="noopener noreferrer">
                        {documento.recuperacion.espacio.liga}
                      </a>
                    </p>
                  )}
                  {documento.recuperacion.espacio.codigo && (
                    <p className="text-gray-600">Código: {documento.recuperacion.espacio.codigo}</p>
                  )}
                </div>
              )}
            </div>
          ) : (
            <p className="text-sm text-gray-500">Los requisitos de recuperación aún no están disponibles.</p>
          )}
        </div>
      )}
    </div>
